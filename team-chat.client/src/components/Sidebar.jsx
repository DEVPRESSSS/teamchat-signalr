import { Radio, MessageSquare } from 'lucide-react';
import { NavLink } from "react-router-dom"
import { useLogout } from '../hooks/useLogout';
import { useAuth } from '../context/authContext';
import Avatar from './Avatar';

const paths = [
    { to: "/userdashboard", name: "Messages", icon: MessageSquare },
];

function Sidebar() {
    const { handleConfirmation } = useLogout();
    const { user } = useAuth();

    return (
        <aside className="flex h-full w-16 flex-col items-center justify-between border-r border-zinc-800 bg-zinc-900 py-4">
            <div
                aria-hidden="true"
                className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-500 text-white"
            >
                <Radio size={16} />
            </div>

            <nav aria-label="Main" className="flex flex-col gap-2">
                {paths.map(({ to, name, icon: Icon }) => (
                    <NavLink
                        key={to}
                        to={to}
                        aria-label={name}
                        title={name}
                        className={({ isActive }) =>
                            `flex h-9 w-9 items-center justify-center rounded-lg transition-colors ${isActive
                                ? "bg-zinc-800 text-white"
                                : "text-zinc-400 hover:bg-zinc-800 hover:text-white"
                            }`
                        }
                    >
                        <Icon size={18} />
                    </NavLink>
                ))}
            </nav>

            <button
                type="button"
                onClick={handleConfirmation}
                aria-label="Log out"
                title="Log out"
                className="rounded-full ring-2 ring-transparent transition-shadow hover:ring-zinc-600"
            >
                <Avatar name={String(user ?? "")} />
            </button>
        </aside>
    );
}

export default Sidebar;